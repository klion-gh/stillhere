...
Иногда баг воспроизводится только у кого-то одного — звонок не соединяется, push не приходит, — и по описанию причину не найти. На этот случай узел умеет вести журнал: что делали приложения и что при этом видел сервер.

**Запись выключена, пока её явно не включат.** В обычной работе приложения ничего никуда не отправляют.

```bash
cd /root/stillhere/docker
./diagnostics.sh on          # начать запись
...
Подключённые приложения подхватывают переключение примерно за 15 секунд — перезапускать ничего не нужно, звонки не рвутся. Записи хранятся **3 дня** и удаляются автоматически, даже если запись давно выключили.

Переключатель работает внутри контейнера, напрямую с базой узла: у него нет HTTP-эндпоинта, поэтому включить чужую диагностику снаружи нельзя в принципе.

Что попадает в журнал: строки собственного лога приложения (подключение сокета, этапы звонка, состояние WebRTC, ошибки) и события узла (кто подключился, кому и что переслано, дошло ли). Тексты сообщений и аудио **не** записываются. Тем не менее это журнал действий ваших пользователей — включайте, когда он нужен, и выключайте после.
