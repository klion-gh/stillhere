...
tools/      генерация иконок из логотипа, служебные скрипты
install.sh  one-liner установщик узла
```

Каждый файл исходников открывается комментарием о том, что это и зачем. Как части связаны между собой — [ARCHITECTURE.md](ARCHITECTURE.md).

## Что есть и чего пока нет

Есть: пейринг с узлом по паролю, сохранённые узлы и аккаунты плитками (повторный вход без пароля), 1:1 текстовый чат с историей, 1:1 аудиозвонки через WebRTC (STUN/TURN для обхода NAT), push-уведомления через собственный Firebase каждого узла — звонки и сообщения доходят и при закрытом приложении, профиль с аватаркой и сменой тега, шесть палитр и восемь анимированных фонов, обновление внутри приложения, TLS без домена через self-signed + TOFU pinning, диагностический журнал на узле (выключен по умолчанию).

Пока нет: групповые чаты, передача файлов, видеозвонки.
...
Нативные обёртки `android/` и `windows/` лежат в репозитории и содержат правки, которых `flutter create` не делает: разрешения на микрофон, receiver для кнопок уведомлений, правила ProGuard и `keep.xml`. Пересоздавать их не нужно.
...
Для справки, разрешения, без которых звонки не работают (уже прописаны в манифесте):
...
Каждый файл исходников начинается с комментария о том, что это и зачем. Карта репозитория и решения, которые не видны внутри одного файла, — в [ARCHITECTURE.md](ARCHITECTURE.md).
